...
**思路：**二分查找。nums[mid] > nums[r]时，l = mid + 1；nums[mid] <= nums[r]时，r--。
...
**思路：**双向**单调队列**。队列头为最大值，每次滑动时，将若滑动窗口的最左值等于队列头，说明该队列头需要被移除了，然后将滑动窗口新加入的最右值加入到队列中保证单调。
