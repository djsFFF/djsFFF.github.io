...
每个结点会根据幂次定律（越大的数出现的概率越小）随机生成多个层，每个层中保存了下一个**同级层**的指针及其跨度。
...
- String：值可以是string，也可以是数字、二进制数据等。底层数据结构可以是**int，raw(SDS)或embstr**。

- Hash：存储键值对，适合存储Java对象。底层数据结构可以是**ziplist或hashtable**。
- List：双向链表，按插入顺序排序。底层数据结构可以是**ziplist或linkedlist**。

- Set：String类型的无序去重集合。底层数据结构可以是**intset或hashtable**。

- Zset（Sorted Set）：在Set的基础上增加了一个score，使得集合中的元素可以按score有序排列。底层数据结构可以是**ziplist或skiplist**。当底层结构为skiplist时，zset中有一个字典用于保存key到score的映射，且key和score是共享跳跃表中的数据，因此不需要额外的内存空间。
...
- 从服务器**只有在接到主服务器发来的DEL命令时才会删除过期键**。在执行客户端发送的请求时，不会考虑键是否已经过期。
...
若服务器在启动时优先检测是否开启AOF持久化，若没有则检测RDB文件并自动载入RDB文件。
...
- BGSAVE：bgsave会派生出一个子进程（fork()方法），然后由子进程负责创建RDB文件。在bgsave执行期间，会拒绝其他的save和bgsave命令。
  ...
  以下命令每100ms检查是否满足保存条件，满足时执行**BGSAVE**命令。

  - save 900 1：在15分钟内，至少1个key发生变化，创建快照。
  - save 300 10：在5分钟内，至少10个key发生变化，创建快照。
  - save 60 10000：在1分钟内，至少10000个key发生变化，创建快照。
...
1. **命令追加：**服务器执行一个写命令，并将该命令追加到服务器状态的aof_buf缓冲区。
...
说明：在操作系统中，当用户调研write()函数时，操作系统通常会将写入数据暂时**保存在一个内存缓冲区**中，可以通过fsync和fdatasync两个**同步函数**强制让操作系统立即将缓冲区中的数据写入到硬盘，保证安全性。
...
AOF文件会随着时间越来越大，因此需要对AOF文件进行重写来减少一些冗余的命令。首先从数据库中读取键现在的值，然后用一条命令去记录键值对，代替之前记录这个键值对的多条命令。

AOF重写是在子进程中执行的，Redis服务器会将AOF重写期间执行的命令同时发送到**AOF缓冲区和AOF重写缓冲**区。当子进程完成重写任务后，父进程会将AOF重写缓冲区里的所有内容写入新AOF文件中，然后原子地覆盖原本的AOF文件。
...
当从服务器初次对主服务器进行复制时，主服务器会将自己的运行ID传送给从服务器保存，以便从服务器断线重连后进行主服务器确认。
...
当Sentinel发送命令`SENTINEL is-master-down-by-addr <ip> <port> <current_epoch> <runid>`从其他Sentinel询问到足够数量的主观下线判定后，会将该服务器判定为客观下线。
...
握手成功时，将ip和port指定的节点加入到当前节点所在的集群中，握手过程：
...
**集群的整个数据库被分为16384（2^14）个槽（slot）**，数据库中的每个键都属于其中某一个slot，当所有slot都有结点负责处理时，集群处于上线状态，否则处于下线状态（fail）。

- 节点结构中的clusterNode.slots是一个16384位的二进制数组，记录了结点负责处理哪些slot。
- 集群状态结构中的clusterState.slots是一个节点链表，记录了集群中16384个slot分别分配给了哪个节点。

## 集群命令执行过程

当客户端向节点发送与某个键相关的命令时，节点得到键所属的slot，若该slot指派给了当前结点，则直接执行该命令，否则会向客户端返回一个`MOVED <slot> <ip>:<port>`错误，指引客户端redirect到正确的结点，并再次发送命令。
...
节点会用clusterState.slots_to_keys来保存slot和键之间的关系，**跳跃表**的**score为slot号，member为键**。
...
如果在重新分片期间，客户端需要访问一个属于正在迁移的slot的键，那么源节点首先在自己的数据库中进行查找，若没有找到，则返回一个`ASK <slot> <ip>:<port>`，指引客户端redirect到目标阶段执行命令。
...
命令`CLUSTER REPLICATE <node_id>`可以让当前结点称为node_id的从节点，并开始对主节点进行复制（相当于执行SLAVEOF命令）。
...
模式的订阅关系都被保存在服务器状态的**pubsub_patterns链表**里面，链表的每个节点记录了被订阅的模式和客户端。PSUBSCRIBE和PUNSUBSCRIBE命令分别添加和删除一个结点。
...
Redis事务队列中的命令要么全部执行，要么一个都不执行。但是Redis不支持事务回滚，即使事务队列中某个命令执行出现了错误，整个事务也会继续执行下去。
...
Redis使用单线程模式来执行事务，因此Redis的隔离级别总是串行化的。
